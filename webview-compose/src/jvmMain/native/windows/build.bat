@echo off
REM Builds compose_webview_windows.dll (WebView2 + DComp) for the
REM ComposeNativeWebView Windows backend.
REM
REM Pattern mirrors Nucleus examples/tao-demo windows/build.bat.
REM
...
REM
REM Outputs:
REM   - compose_webview_windows.dll  -> resources/nucleus/native/win32-{x64,aarch64}/
REM   - WebView2Loader.dll           -> same dir (sidecar)
...
set "SOURCES=jni_bridge.cpp view_signals.cpp view_input.cpp view_lifecycle.cpp navigation.cpp javascript.cpp cookies.cpp screenshot.cpp"

echo.
echo === Building compose_webview_windows.dll (x64) ===
pushd "%SCRIPT_DIR%"
cl /LD /O2 /EHsc /std:c++17 /nologo /MT ^
    ...
    /I"%WV2_INCLUDE%" ^
    %SOURCES% ^
    /Fe:"%OUT_DIR_X64%\compose_webview_windows.dll" ^
...
echo.
echo === Building compose_webview_windows.dll (ARM64) ===
pushd "%SCRIPT_DIR%"
cl /LD /O2 /EHsc /std:c++17 /nologo /MT ^
    /D_UNICODE /DUNICODE ^
    /I"%JAVA_HOME%\include" /I"%JAVA_HOME%\include\win32" ^
    /I"%WV2_INCLUDE%" ^
    %SOURCES% ^
    /Fe:"%OUT_DIR_ARM64%\compose_webview_windows.dll" ^
    /link kernel32.lib user32.lib ole32.lib oleaut32.lib dcomp.lib
set "CL_RC=%ERRORLEVEL%"
popd
if not "%CL_RC%"=="0" (
...
if exist "%OUT_DIR_X64%\compose_webview_windows.dll"   echo   %OUT_DIR_X64%\compose_webview_windows.dll
if exist "%OUT_DIR_X64%\WebView2Loader.dll"            echo   %OUT_DIR_X64%\WebView2Loader.dll
if exist "%OUT_DIR_ARM64%\compose_webview_windows.dll" echo   %OUT_DIR_ARM64%\compose_webview_windows.dll
